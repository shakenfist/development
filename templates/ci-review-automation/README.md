...
Every one of those projects now calls the reusable `pr-auto-review.yml`
for its automatic review; the in-CI `automated_reviewer` job the
migration section above describes no longer exists anywhere, and that
section is kept for the repositories which have not adopted any of this
yet. `development` and `ryll` call it from `ci.yml`, everyone else from
