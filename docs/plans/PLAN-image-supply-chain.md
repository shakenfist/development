...
is that it closes and nobody notices, so the eight deferred sites
...
`grep -rn 'shakenfist#4280'`, anchored, and not as `grep -rn
4280`**: run bare on 2026-09-22, the unanchored form returns one
real hit and three lines of
...
- [ ] `grep -rn --exclude='*.md' --exclude-dir=.git
      ...
      **and the eight under-cloud sites shakenfist#4280 blocks**:
      ...
      instead of naming the issue, so before 4f this bullet has
      seven of eight and the set grep has seven. The substring
...
phase started, and grew three new ones while doing it. The risk
moved next door: phase 4's survey found a second class of consumer
the criterion cannot see, and being
