...
That is mitigated by the gate on a published `ci-images/debian-13`
blob, by 4f triggering kerbside's `sf-e2e-functional` after merge and
reading it to completion, and by the default move being its own
commit so the revert is one commit -- which stays true only while it
is the newest landing in that file.
...
- [ ] `grep -E '^[[:space:]]*-[[:space:]]*debian-12'` over the
      ...
      The scope is the whole organisation rather than the thirteen,
      because a repository that has already migrated is where a
      declaration outlives its last user -- `kerbside-patches` was
      exactly that when this phase was planned.
- [ ] `grep -rn --include='*.py' --include='*.yml' --include='*.yaml'
      --include='*.sh' "ci-images/debian-12"` over fresh clones of
      ...
      one. The `--include` list is what keeps the grep off prose --
      this plan names the string a dozen times, and `development` is
      in the clone set -- and it is the same exemption `eol-distro`
      makes for a document describing a migration. `private-ci` is
      in the clone set deliberately: it is excluded from the
      criterion, so it is exactly the repository a criterion-shaped
      check would miss.
...
- [ ] The same grep for `/srv/ci/debian:12` returns nothing, and
      ...
      bullet asking for it would pass vacuously.
