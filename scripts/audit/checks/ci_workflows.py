...
keyed on, whether a lane that costs a VM is filtered.
"""
...
def merge_queue_is_serial(repo_name, org, github=None):
    ...
    """
    try:
        client = github if github is not None else GhCli()
        branch = client.api(
    ...
    def run(self, repo):
        ...
        if repo.props['is_docs_only']:
            ...
            if not check_file_exists(
                repo.path, '.github/workflows/pr-re-review.yml'
            ):
                missing.append('pr-re-review.yml')
            deployed = carries_retired_comment_addresser(repo.path)
            if deployed:
                return self.fail(addresser_retired_detail(deployed))
            if pr_re_review_open_codes_the_trigger(repo.path):
                return self.fail(
...
class WorkflowPermissions(Check):
    id = 'workflow-permissions'
    spec = 'docs/audits/workflow-standards.md'
    template = None
    issue_title = 'Workflow standards'
    column = 'Permissions'
    ...
    def run(self, repo):
        ...
        missing = [
            ...
            if not workflow_has_permissions(repo.path, wf)
        ]

        if missing:
            return self.fail(
                f'{len(missing)} workflow(s) missing top-level '
                f'permissions: {", ".join(sorted(missing))}', missing=sorted(missing))
        return self.ok(
            f'All {len(workflows)} workflows have '
            f'top-level permissions')


class PreCommitConfig(Check):
    id = 'pre-commit-config'
    spec = 'docs/audits/workflow-standards.md'
    template = None
    issue_title = 'Workflow standards (linting)'
    column = 'Linting'

    def run(self, repo):
        """Check for .pre-commit-config.yaml."""
        if not repo.exists('.pre-commit-config.yaml'):
            return self.fail('Missing .pre-commit-config.yaml')
        return self.ok('.pre-commit-config.yaml exists')


class DevpiFallback(Check):
    id = 'devpi-fallback'
    spec = 'docs/audits/workflow-standards.md'
    template = None
    issue_title = 'Workflow standards (devpi cache fallback)'
    column = 'devpi fallback'
    ...
    def run(self, repo):
        ...
        if not devpi_seen:
            return self.skip('No jobs use the local devpi cache')
        if offenders:
            return self.fail(
        ...
        return self.ok('All devpi-backed jobs set a PIP_EXTRA_INDEX_URL fallback')


class DevpiStaleIp(Check):
    id = 'devpi-stale-ip'
    spec = 'docs/audits/workflow-standards.md'
    template = None
    issue_title = 'Workflow standards (devpi cache address)'
    column = 'devpi IP'
    ...
    def run(self, repo):
        ...
        return self.ok('No references to the retired devpi address')


class ExpensiveLanePathFilter(Check):
    id = 'expensive-lane-path-filter'
    spec = 'docs/audits/expensive-lane-path-filter.md'
    template = None
    issue_title = 'Expensive lane path filtering'
    ...
    def run(self, repo):
        ...
        if repo.exists('.vscode/review-scope.toml'):
...
class MergeGroupCancellation(Check):
    id = 'merge-group-cancellation'
    spec = 'docs/audits/merge-group-cancellation.md'
    template = None
    issue_title = 'Merge group run cancellation'
    ...
    def run(self, repo):
        ...
        if (keyed_on_base_ref
                and merge_queue_is_serial(repo.name, repo.org, repo.github) is False):
            offenders.append(
...
class SecretScanningCi(Check):
    id = 'secret-scanning-ci'
    spec = 'docs/audits/secret-handling.md'
    template = None
    issue_title = 'Secret scanning in CI'
    ...
    def run(self, repo):
        ...
        """
        if repo.props['is_docs_only']:
            return self.skip('Documentation-only repository')
        ...
        for workflow in workflows:
            path = os.path.join(repo.path, '.github', 'workflows', workflow)
            try:
            ...
            for scanner in SECRET_SCANNERS:
                if scanner in content:
                    return self.ok(f'{scanner} runs in {workflow}')
