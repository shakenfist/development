...
# A runner label only names a runner when it sits where YAML puts a
# value: after "runs-on:", as an element of a "[...]" list, or as a
# "- " item in a matrix. The same text inside a shell command is not a
# runner reference -- shakenfist/actions ships a step which uploads an
# image artifact named "ubuntu-2004", and reporting that asked someone
# to mark a deliberate exception on a line which never described a
# runner at all.
