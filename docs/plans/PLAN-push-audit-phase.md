...
| 4. Fleet backfill | Complete | `fd0678c` (#113) |
...
index into a status table on 2026-09-10 (`7600cc6db`, "Register
...
worked example has moved repositories.
...
reading the tables directly -- every master plan in every clone,
every table carrying both a `Status` and a `Merged` column, every
row whose status is terminal and whose `Merged` cell is empty:

```python
TERMINAL = {'complete', 'abandoned', 'superseded'}
# for each row of each table carrying both columns:
if status in TERMINAL and not merged:
    print(repo, plan, lineno, phase)
```

Fleet-wide that names exactly one row: instar's
`PLAN-differencing.md` phase 4, which merged as
[instar#563](https://github.com/shakenfist/instar/pull/563)
(`f981374d0`) on 2026-09-14, three days after 4e. instar's own plan
...
housekeeping rather than a backfill this phase missed -- it is in
the same plan file as the failure above, and the same instar change
closes both. Every other landed phase in the fleet records a range.
...
Nothing further needs re-editing in a later step.
...
* `plan-audit-phase` passes in shakenfist, ryll, kerbside,
  ...
  **Met in every repository this phase swept, and not met in
  instar**, which *What the sweep found* records in full. Two
  clauses of this criterion as first written turned out to describe
  a fleet that had moved: occystrap is named as seven
  terminal-status plans rather than two statusless ones, because it
  rewrote its index on 2026-09-10 and the statusless bucket is now
  exercised by ryll instead; and instar fails on a second carrier
  that appeared the day after the survey, already filed as
  instar#554 and outside every sweep step's brief. The criterion is
  restated above to what was measured rather than left claiming a
  pass that is not there.
...
**Merged:** `fd0678c` (#113), carrying steps 4a and 4b, and the pull
request carrying step 4f. 4f's own merge commit is recorded by the
...
|------|------------|--------------|--------------|
| 4c | shakenfist | #4160 | `108987e9c` |
| 4d | divergulent | #109 | `2deb2af6a` |
| 4e | ryll | #371 | `c52c10611` |
| 4e | instar | #560 | `3a297f7ed` |
| 4e | kerbside | #419 | `1d9d28dd1` |
| 4e | client-python-k3s | #57 | `0e88d41cd` |
...
* **The survey enumerated eight repositories; the fleet is
  ...
  previous phase happened to visit.
